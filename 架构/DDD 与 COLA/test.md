...
- [二.基础技术](#-----)
  ...
  * [2.Cgroups](#2cgroups)
    + [2.1 Cgroups中的3个组件](#21-cgroups--3---)
    + [2.2 Cgroup文件系统](#22-cgroup----)
- [三.构造容器](#-----)
  * [1.proc文件系统](#1proc----)
  * [2.mydocker文件分布结构](#2mydocker------)
  * [3.实现run命令](#3--run--)
    + [1）main](#1-main)
    + [2）runCommand](#2-runcommand)
    + [3）Run](#3-run)
    + [4）NewParentProcess](#4-newparentprocess)
    + [5）initCommand](#5-initcommand)
    + [6）RunContainerInitProcess](#6-runcontainerinitprocess)
    + [7）setUpMount](#7-setupmount)
    + [8）pivotRoot](#8-pivotroot)
  * [4.增加容器资源限制](#4--------)
    + [4.1 子系统限制资源](#41--------)
    + [4.2 查找cgroup路径](#42---cgroup--)
    + [4.3 管理每个cgroup](#43-----cgroup)
    + [4.4 带资源限制的容器](#44---------)
- [四.构造镜像](#-----)
  * [1.联合挂载与volume挂载](#1-----volume--)
    - [1）处理镜像层和容器层](#1----------)
    - [2）处理volume卷](#2---volume-)
  * [2.volume卸载与容器文件系统卸载](#2volume-----------)
    - [1）处理volume卷](#1---volume-)
    - [2）处理镜像层和容器层](#2----------)
  * [3.简单的镜像打包](#3-------)
